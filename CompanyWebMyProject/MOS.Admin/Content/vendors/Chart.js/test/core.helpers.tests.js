describe('Core helper tests', function() {
	...
	it('should correctly determine if two numbers are essentially equal', function() {
		expect(helpers.alCompanyWebtEquals(0, Number.EPSILON, 2 * Number.EPSILON)).toBe(true);
		expect(helpers.alCompanyWebtEquals(1, 1.1, 0.0001)).toBe(false);
		expect(helpers.alCompanyWebtEquals(1e30, 1e30 + Number.EPSILON, 0)).toBe(false);
		expect(helpers.alCompanyWebtEquals(1e30, 1e30 + Number.EPSILON, 2 * Number.EPSILON)).toBe(true);
	});
